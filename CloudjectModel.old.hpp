...
protected:
	...
	float match(LFCloudject<PointT,SignatureT> c)
	{
		...
		if (c.getType() == LFCloudject<PointT,SignatureT>::OneView)
		{
			...
			if (this->getSizePenalty() == this->SizePenalty::NumOfPoints)
			{
...
public:
	...
	{
		for (int i = 0; i < this->m_NViews; i++)
		{
			DescriptorPtr pDescriptor (new Descriptor);
			describeView(this->m_ViewClouds[i], normalRadius, fpfhRadius, *pDescriptor);
			this->m_ViewsDescriptors.push_back(pDescriptor);
		}
	}
...
private:
					  ...
					  Descriptor& descriptor)
	{
		...
		typename pcl::search::KdTree<PointT>::Ptr treeNormals (new pcl::search::KdTree<PointT> ());
		ne.setSearchMethod (treeNormals);
		...
		// Its content will be filled inside the object, based on the given input dataset (as no other search surface is given).
        typename pcl::search::KdTree<PointT>::Ptr treeFPFH (new pcl::search::KdTree<PointT> ());
...
private:
					  ...
					  Descriptor& descriptor)
	{
		...
		// Create an empty kdtree representation, and pass it to the FPFH estimation object.
		// Its content will be filled inside the object, based on the given input dataset (as no other search surface is given).
		typename pcl::search::KdTree<PointT>::Ptr treePFHRGB (new pcl::search::KdTree<PointT> ());
		pfhrgb.setSearchMethod (treePFHRGB);
